...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(`❌ Error checking partner view: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
    ...
    try {
      ...
      console.log(`❌ Error checking partner manage: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }

    // Test 3: Can view a different partner (should be false)
    try {
      const canViewOtherPartner = await checkPermission(userId, 'view', 'partner:different-partner-id');
      ...
      console.log(`❌ Error checking other partner view: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
    ...
    try {
      ...
      console.log(`❌ Error checking platform view: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
