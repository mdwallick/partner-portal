...
export async function GET(request: NextRequest) {
  try {
    ...
    const gamesWithCount = games.map(g => ({
      ...g,
      client_count: g.client_ids.filter(c => c.status === "active").length,
    }))
...
export async function POST(request: NextRequest) {
  try {
    ...
    const partner = await prisma.partner.findFirst({
      where: { id: { in: activePartnerIds }, type: "game_studio" },
    })
    if (!partner) {
    ...
    await checkPermission(`partner:${partner.id}`, "owner", `game:${gameId}`)
