...
export async function GET(request: NextRequest) {
  try {
    ...
    const accessiblePartnerIds = await listObjects(
      `user:${user.sub}`,
      'can_view',
      'partner'
    );
    
    console.log(`🗄️ FGA returned ${accessiblePartnerIds.length} partner IDs that 👤 ${user.email} is allowed to view:`, accessiblePartnerIds);
    ...
    const partners = await sql`
      ...
      WHERE id = ANY(${accessiblePartnerIds})
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newPartner = await sql`
      INSERT INTO partners (id, name, type, organization_id, logo_url)
      VALUES (${partnerId}, ${name}, ${type}, ${groupId || null}, ${logo_url || null})
      RETURNING *
    `;
    ...
    return NextResponse.json(newPartner[0]);
  } catch (error) {
