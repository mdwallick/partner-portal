...
export async function PUT(
    ...
    try {
        ...
        if (display_name !== undefined && display_name !== currentPU.user?.display_name) {
            console.log(`Updating Auth0 user display name from "${currentPU.user?.display_name}" to "${display_name}"`);
...
export async function DELETE(
    ...
    try {
        ...
        if (partnerUser.role === 'can_admin') {
            const adminCount = await prisma.partnerUser.count({ where: { partner_id: partnerId, role: 'can_admin', status: 'active' } });
        ...
        try {
            ...
            console.log(`✅ Removed FGA tuple: user:${partnerUser.user?.auth0_user_id} ${fgaRoleToDelete} partner:${partnerId}`);
        } catch (error) {
        ...
        console.log(`Checking if Auth0 user ${partnerUser.user?.auth0_user_id} is part of other partners...`);
        const otherPartnerCount = await prisma.partnerUser.count({ where: { user: { auth0_user_id: partnerUser.user?.auth0_user_id || '' } } });

        console.log(`Found ${otherPartnerCount} other partner relationships for Auth0 user ${partnerUser.user?.auth0_user_id}`);
        ...
        if (otherPartnerCount === 0) {
            ...
            try {
                console.log(`Attempting to permanently delete Okta user: ${partnerUser.user?.auth0_user_id}`);
