...
export default function TokenDebugger() {
  ...
  const formatToken = (token: string) => {
    if (!token || token === 'Not available' || token === 'Failed to fetch' || token === 'Error fetching token') {
      return token;
    }
      ...
      <div className="bg-gray-900 text-white border-t border-gray-700">
        ...
        >
          <div className="flex items-center space-x-2">
            <span className="text-sm font-medium">🔐 Okta Token Debugger</span>
        ...
        {isExpanded && (
          <div className="px-4 pb-4 space-y-3">
                        ...
                        <div>
                          ...
                          {showDecoded && tokens.idToken && tokens.idToken !== 'Not available' && tokens.idToken !== 'Failed to fetch' && tokens.idToken !== 'Error fetching' && (
                            <div className="mt-2">
                              ...
                              <div className="bg-gray-900 p-2 rounded text-xs">
                                ...
                                <pre className="text-green-300 overflow-auto">{JSON.stringify(decodeJWT(tokens.idToken)?.header, null, 2)}</pre>
                                <div className="text-yellow-300 mb-1 mt-2">Payload:</div>
                                <pre className="text-blue-300 overflow-auto">{JSON.stringify(decodeJWT(tokens.idToken)?.payload, null, 2)}</pre>
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          {showDecoded && tokens.accessToken && tokens.accessToken !== 'Not available' && tokens.accessToken !== 'Failed to fetch' && tokens.accessToken !== 'Error fetching' && (
                            <div className="mt-2">
                              <div className="text-xs text-gray-400 mb-1">Decoded Access Token:</div>
                              <div className="bg-gray-900 p-2 rounded text-xs">
                                <div className="text-yellow-300 mb-1">Header:</div>
                                <pre className="text-green-300 overflow-auto">{JSON.stringify(decodeJWT(tokens.accessToken)?.header, null, 2)}</pre>
                                <div className="text-yellow-300 mb-1 mt-2">Payload:</div>
                                <pre className="text-blue-300 overflow-auto">{JSON.stringify(decodeJWT(tokens.accessToken)?.payload, null, 2)}</pre>
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          {showDecoded && tokens.refreshToken && tokens.refreshToken !== 'Not available' && tokens.refreshToken !== 'Failed to fetch' && tokens.refreshToken !== 'Error fetching' && (
                            <div className="mt-2">
                              <div className="text-xs text-gray-400 mb-1">Decoded Refresh Token:</div>
                              <div className="bg-gray-900 p-2 rounded text-xs">
                                <div className="text-yellow-300 mb-1">Header:</div>
                                <pre className="text-green-300 overflow-auto">{JSON.stringify(decodeJWT(tokens.refreshToken)?.header, null, 2)}</pre>
                                <div className="text-yellow-300 mb-1 mt-2">Payload:</div>
                                <pre className="text-blue-300 overflow-auto">{JSON.stringify(decodeJWT(tokens.refreshToken)?.payload, null, 2)}</pre>
                              </div>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="text-xs text-gray-400">
                        ...
                        <div className="pt-2 border-t border-gray-700">
                          <div className="text-xs text-green-300">
                            <div><strong>✅ Tokens are now live from /api/debug-session!</strong></div>
                            <div>🔑 Copy the Access Token above to use in your API calls</div>
                            <div>📋 Use in Authorization header: <code className="bg-gray-800 px-1 rounded">Bearer &lt;accessToken&gt;</code></div>
                          </div>
                        </div>
          </div>
        )}
