...
class OktaManagementAPI {
  ...
  async createGroup(
    ...
    try {
      ...
      throw new Error(
        `Failed to create Okta group: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
  ...
  async getAllGroups(): Promise<OktaOrganization[]> {
    try {
      ...
      throw new Error(
        `Failed to get Okta groups: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
  ...
  async createUser(
    ...
    try {
      ...
      throw new Error(
        `Failed to create Okta user: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
  ...
  async removeUserFromGroup(userId: string, groupId: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to remove user from group: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
  ...
  async getGroupUsers(groupId: string): Promise<OktaUser[]> {
    try {
      ...
      throw new Error(
        `Failed to get group users: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
  ...
  async deleteOrganization(groupId: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to delete Okta group: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
  ...
  async updateGroup(groupId: string, name: string, description?: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to update Okta group: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }

  // Create an OIDC Native Application
  async createOIDCNativeApp(
    name: string,
    redirectUris?: string[]
    ...
    try {
      const appData = {
        ...
        settings: {
          oauthClient: {
            ...
            response_types: ["code"],
            redirect_uris: redirectUris || ["com.example.app:/callback"],
      ...
      throw new Error(
        `Failed to create Okta OIDC native app: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }

  // Create an OIDC Web Application
  async createOIDCWebApp(
    name: string,
    redirectUris?: string[]
  ): Promise<{ clientId: string; clientSecret: string }> {
    try {
      const appData = {
        ...
        settings: {
          oauthClient: {
            ...
            grant_types: ["authorization_code", "refresh_token"],
            response_types: ["code"],
            redirect_uris: redirectUris || ["http://localhost:3000/callback"],
      ...
      throw new Error(
        `Failed to create Okta OIDC Web app: ${error instanceof Error ? error.message : "Unknown error"}`
      )
    }
  }
}
