...
export class Auth0ManagementAPI {
    ...
    async createUser(params: {
        ...
        if (!connection) throw new Error('Missing DB connection. Set AUTH0_DB_CONNECTION or pass connection param.');
    ...
    async listUsers(options?: { q?: string; page?: number; perPage?: number }): Promise<Auth0UserSummary[]> {
    ...
    async updateUser(userId: string, updates: {
        ...
        const { data: u } = await this.mgmt.users.update({ id: userId }, {
            email: updates.email,
            name: updates.name,
    ...
    async listOrganizations(options?: { page?: number; perPage?: number }): Promise<Auth0OrganizationSummary[]> {
        const res = await this.mgmt.organizations.getAll({ page: options?.page, per_page: options?.perPage } as any);
    ...
    async updateOrganization(orgId: string, updates: {
        ...
        const { data: o } = await this.mgmt.organizations.update({ id: orgId } as any, {
            name: updates.name,
    ...
    async addUserToOrganization(orgId: string, userId: string): Promise<void> {
        await this.mgmt.organizations.addMembers({ id: orgId } as any, { members: [{ user_id: userId }] } as any);
    }

    async removeUserFromOrganization(orgId: string, userId: string): Promise<void> {
        await this.mgmt.organizations.deleteMembers({ id: orgId } as any, { members: [{ user_id: userId }] } as any);
    }

    async listOrganizationMembers(orgId: string, options?: { page?: number; perPage?: number }): Promise<Auth0UserSummary[]> {
        const res = await this.mgmt.organizations.getMembers({ id: orgId } as any, {
            page: options?.page,
            per_page: options?.perPage,
        } as any);
