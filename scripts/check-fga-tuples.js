...
async function checkFgaTuples() {
  ...
  try {
    ...
    const { data: { allowed } } = await fgaClient.check({
      tuple_key: {
        user: `user:${userId}`,
        relation: 'view',
    ...
    const { data: { allowed: canManage } } = await fgaClient.check({
      tuple_key: {
        user: `user:${userId}`,
        relation: 'manage',
        object: `partner:${partnerId}`
    ...
    const { data: { allowed: canViewOther } } = await fgaClient.check({
      tuple_key: {
        user: `user:${userId}`,
        relation: 'view',
